      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-cyan-600 dark:bg-teal-500 block rounded-3xl"
          ...
          <Card>
            {item.ico && <span className="block dark:bg-teal-500/25 bg-cyan-500/25 w-max p-2 rounded-xl">{item.ico}</span>}
